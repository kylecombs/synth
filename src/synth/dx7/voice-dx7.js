...
class FMVoice {
	...
	static updateFrequency(operatorIndex) {
		if (!params || !params.operators || !params.operators[operatorIndex]) return;
	...
	static setPan(operatorIndex, value) {
		if (!params || !params.operators || !params.operators[operatorIndex]) {
			return;
		}
		const op = params.operators[operatorIndex];
	...
	render() {
		const algorithmIdx = params.algorithm - 1;
		...
		// Debug first render
		if (!this._debugged) {
			console.log('Voice render - Algorithm:', params.algorithm, 'OutputMix:', outputMix);
			this._debugged = true;
		}
		...
		for (let k = 0, length = outputMix.length; k < length; k++) {
			if (params.operators[outputMix[k]].enabled) {
				...
				const carrierParams = params.operators[outputMix[k]];
	...
	isFinished() {
		const outputMix = ALGORITHMS[params.algorithm - 1].outputMix;
